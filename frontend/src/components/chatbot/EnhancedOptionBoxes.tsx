// src/components/chatbot/EnhancedOptionBoxes.tsx

import { MapPin, Clock, DollarSign, Star, Plane, Building, Calendar } from 'lucide-react';
import { Button } from '../ui/button';
import { HolidayOption } from '../../types';

interface EnhancedOptionBoxesProps {
  options: HolidayOption[];
  onSelectOption: (option: HolidayOption) => void;
}

export default function EnhancedOptionBoxes({ options, onSelectOption }: EnhancedOptionBoxesProps) {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 mt-4 ml-12">
      {options.map((option) => (
        <div
          key={option.id}
